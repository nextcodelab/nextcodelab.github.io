...
!(function(a, b, c, d) {
  ...
  (fb.prototype = {
    ...
    remove: function(a) {
      ...
      return (
        (a = this.get(a)), b.splice(s(b, a), 1), this.touchAction.update(), this
      );
    },
    on: function(a, b) {
      var c = this.handlers;
...
!(function($) {
  ...
  $.fn.zenith = function(options) {
    ...
    if (settings.layout === "slider") {
      this.find(settings.markup[0] + " " + settings.markup[1]).addClass(
        "animated"
      );
    }
    ...
    if (
      ...
      layout === "hand" ||
      layout === "screen" ||
      (layout === "slider" && direction === "horizontal")
    ) {
      ...
      layout === "screen" ||
      (layout === "slider" && direction === "vertical")
    ) {
      if (settings.animation === "Slide") {
    ...
    var screenSlide = {
      next: function() {
        ...
        var current = parent.find(
          holder + " " + item + '[data-index="' + currAct + '"]'
        );
        var bulletA = parent.find(bts + ".active");
        var next = parent.find(
          holder + " " + item + '[data-index="' + parseInt(currAct - 1) + '"]'
        );
        ...
        if (wait === 0) {
          ...
          current
            .removeClass(animation["in_dir1"] + " " + animation["in_dir2"])
            .addClass(animation["out_dir2"]);
      ...
      bullets: function(el) {
        ...
        if (settings.layout != "slider") {
          ...
          var currSlide = parent.find(
            '#mac-book .hgi[data-index="' + slideIdx + '"]'
          );
          var nextSlide = parent.find(
            '#mac-book .hgi[data-index="' + parseInt(index) + '"]'
          );
        } else {
        ...
        if (!el.hasClass("active") && wait === 0) {
          ...
          parent
            .find(bulItem + '[data-index="' + index + '"]')
            .addClass("active");
          if (parseInt(index) > parseInt(current.data("index"))) {
            currSlide
              .removeClass(animation["in_dir1"] + " " + animation["in_dir2"])
              .addClass("active " + animation["out_dir1"]);
            ...
            currSlide
              .removeClass(animation["in_dir1"] + " " + animation["in_dir2"])
              .addClass("active " + animation["out_dir2"]);
      ...
      init: function() {
        ...
        var item =
          settings.layout === "slider"
            ? settings.markup[0] + " " + settings.markup[1]
            : "#mac-book .hgi";
        var actI = settings.activeIndex;
        var startUp =
          settings.layout === "slider"
            ? parent
                .find(settings.markup[0] + " " + settings.markup[1])
                .eq(actI)
            : parent.find('#mac-book .hgi[data-index="' + actI + '"]');
        if (settings.layout != "slider") {
          var left = parent.find("#mac-book #left");
          var right = parent.find("#mac-book #right");
          ...
          var right = parent.find("#right");
          var firstBul =
            settings.customNavs[0] +
            " " +
            settings.customNavs[1] +
            '[data-index="' +
            actI +
            '"]';
        }
        ...
        if (settings.layout === "screen") {
          ...
          if (settings.customNavs[0] === "#bullet-navs") {
            ...
            parent
              .find(settings.customNavs[0] + " " + settings.customNavs[1])
              .each(function() {
                $(this).attr("data-index", idx);
                idx++;
              });
        ...
        style +=
          ID +
          " #bullet-navs ul li span{background: " +
          settings.bulletsColor +
          "}";
        style +=
          ID +
          " #bullet-navs ul li.active span{background: " +
          settings.bulletsAC +
          "}";
        style +=
          ID + " " + settings.markup[0] + "{height: " + settings.height + "}";
        style +=
          ID + " " + settings.markup[0] + "{width: " + settings.width + "}";
        ...
        style +=
          ...
          " " +
          item +
          ".animated{-webkit-animation-duration: " +
          settings.slideSpeed +
        ...
        if (settings.customNavs[0] === "#bullet-navs") {
          style +=
            ...
            " " +
            settings.customNavs[0] +
            "{height: auto; position: absolute;bottom: 0px; padding: 0 0.5%; padding-top: 2px; width: 90%;}";
        } else {
          style +=
            ID +
            " " +
            settings.customNavs[0] +
            "{height: auto; position: absolute;bottom: 0px; padding: 0 0.5%; padding-top: 2px; width: 100%;}";
        }
      ...
      thumbs: function() {
        ...
        if (settings.fixedThumbs === true) {
          ...
          var chg =
            "#" + ID + " " + settings.markup[0] + " " + settings.customNavs[0];
          if (settings.navPosition === "bottom") {
            $(chg).css("margin-bottom", -padding + "px");
            $("#" + ID + " " + settings.markup[0]).hover(
              function() {
    ...
    var autoplay = {
      ...
      first: function() {
        ...
        var next = zis
          .find(".phone-holder")
          .find('.hgi[data-index="' + parseInt(currActive + 1) + '"]');
        var last = zis
          .find(".phone-holder")
          .find(".hgi")
          .last()
        ...
        if (0 === wait) {
          ...
          zis
            .find(".hgi.active")
            .removeClass(
              "active " + animation["in_dir1"] + " " + animation["in_dir2"]
            );
      ...
      third: function() {
        ...
        var next = parent
          .find(holder)
          .find(item + '[data-index="' + parseInt(currActive + 1) + '"]');
        var back = parent
          .find(holder)
          .find(item + '[data-index="' + parseInt(currActive - 1) + '"]');
        var last = parent
        ...
        var prev = parent
          .find(holder)
          .find(item + '[data-index="' + prevIndex + '"]');
        ...
        if (lastHit === 0 && wait === 0) {
          ...
          if (parseInt(currActive + 1) <= parseInt(last)) {
            parent
              .find(bts + '[data-index="' + parseInt(currActive + 1) + '"]')
              .addClass("active");
            prev
              .removeClass(animation["in_dir1"] + " " + animation["in_dir2"])
              .addClass("active " + animation["out_dir1"]);
        ...
        if (lastHit === 1 && wait === 0) {
          ...
          if (parseInt(currActive - 1) >= 0) {
            ...
            wait = 0;
            parent
              .find(bts + '[data-index="' + parseInt(currActive) + '"]')
              .addClass("active");
          }
        } //if()
      } //third()
    ...
    if (settings.layout === "default") {
      ...
      style +=
        "#" +
        ID +
        " .highlight.active h3:after{background: " +
        settings.activeColor +
        "}";
      style +=
        "#" +
        ID +
        " .highlight.active .fa{background: " +
        settings.activeColor +
        "}";
      ...
      style +=
        ...
        ID +
        " " +
        ".hgi.animated{-webkit-animation-duration: " +
        settings.slideSpeed +
        "ms!important; animation-duration: " +
        settings.slideSpeed +
        "ms!important;}";
      style += "</style>";
      //print styles
      ...
      parent
        .find('.hgi[data-index="' + settings.activeIndex + '"]')
        .addClass("active " + animation["in_dir1"]);
      parent
        .find('.highlight[data-index="' + settings.activeIndex + '"]')
        .addClass("active");
      ...
      parent.find(".highlight").mouseenter(function() {
        if (!$(this).hasClass("active") && wait === 0) {
          ...
          var hgi = parent
            .find(".phone-holder")
            .find('.hgi[data-index="' + hIndex + '"]');
          var prevIndex = parent
            .find(".phone-holder .hgi.active")
            .data("index");
          var prev = parent
            .find(".phone-holder")
            .find('.hgi[data-index="' + prevIndex + '"]');
          parent
            .find(".hgi.active")
            .removeClass(
              animation["in_dir1"] + " " + animation["in_dir2"] + " active"
            );
      ...
      //setting the default active screen
      Parent.find(".hgi").addClass("animated");
      Parent.find('.hgi[data-index="' + settings.activeIndex + '"]').addClass(
        "active " + animation["in_dir1"]
      );
      Parent.find(
        '.highlight[data-index="' + settings.activeIndex + '"]'
      ).addClass("active");
      ...
      Parent.on("mouseenter", ".highlight", function() {
        if (!$(this).hasClass("active") && wait === 0) {
          ...
          if (parseInt(Index) > parseInt(prevIndex)) {
            ...
            parent.find(prev).one(animationEvent, function(event) {
              prev.removeClass(
                animation["out_dir1"] + " " + animation["out_dir2"]
              );
      ...
      if (settings.fullWidth === true) {
        parent
          .find(settings.markup[0])
          .css({ width: "100%", margin: 0, height: $(window).outerHeight() });
        parent.css("margin", 0);
      }
      ...
      parent
        .closest("html")
        .find('head style[data-title="' + ID + '"]')
        .prepend(style);

      var bulEl = settings.customNavs[1];
      var bullet = parent.find(
        settings.customNavs[0] + " " + settings.customNavs[1]
      );
      ...
      //pause autoplay on hover
      if (settings.autoplayPause === true && settings.autoplay === true) {
        parent.find(settings.markup[0]).hover(
          function() {
            clearInterval(play);
          },
          function() {
